...
export const calculateHungarianResults: VotingCampaignCalculator = async (meta) => {
	...
	const profitMatrix = Array.from({ length: meta.group.length }, () =>
	...
	for (let i = 0; i < votes.length; i++) {
		if (votes[i][1].campaignType === "exemption")
			profitMatrix[i] = Array.from({ length: meta.group.length }, () => +Infinity);
		else {
			...
			for (let j = 0; j < vote.topSeats.length; j++)
				profitMatrix[i][vote.topSeats[j] - 1] = vote.topSeats.length - j;
		}
	}
